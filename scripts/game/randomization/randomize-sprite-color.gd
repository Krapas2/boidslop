...
func _apply_color():
	var color_modulation := _get_random_color()
	
	parent.modulate = color_modulation

func _get_random_color() -> Color:
	var hue_shift := randf_range(-hue_shift_range, hue_shift_range)
	var sat_mult := randf_range(-saturation_range, saturation_range)
	var val_mult := randf_range(-value_range, value_range)
